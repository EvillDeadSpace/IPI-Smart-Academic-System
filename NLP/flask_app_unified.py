        ...
        for file_path in possible_paths:
            ...
            if os.path.exists(file_path):
                with open(file_path, encoding="utf-8") as file:
                    content = file.read()
                    print(
                        f"File loaded successfully from {file_path}, content length: {len(content)}"
                    )
                    return content
    ...
    try:
        ...
        for idx in top_indices:
            if similarities[idx] > 0.1:  # Threshold for relevance
                results.append({"content": chunks[idx], "score": float(similarities[idx])})
    ...
    def generate_fallback_response(query, context_text):
        ...
        if any(word in query_lower for word in ["lokacija", "adresa", "gdje", "gde", "nalazi"]):
            location_info = [
                ...
                if any(
                    word in line.lower()
                    for word in ["zmaja", "bosne", "tuzla", "adresa", "lokacija"]
                )
            ]
            if location_info:
        ...
        # Cijena/košta
        if any(
            word in query_lower for word in ["cijena", "cena", "košta", "kosta", "uplata", "novac"]
        ):
            price_info = [
                line
                for line in filtered_lines
        ...
        if any(
            word in query_lower
            for word in ["program", "studij", "smer", "smjer", "smjerovi", "smerovi"]
        ):
            program_info = [
                line
                for line in filtered_lines
        ...
        if any(word in query_lower for word in ["kontakt", "telefon", "email", "mail"]):
            contact_info = [
                ...
                if any(
                    word in line.lower()
                    for word in ["kontakt", "telefon", "email", "mail", "@", "tel", "fax"]
                )
            ]
            if contact_info:
        ...
        if any(word in query_lower for word in ["šta", "sta", "ipi", "akademija", "fakultet"]):
            general_info = [
                ...
                if any(
                    word in line.lower()
                    for word in ["akademija", "fakultet", "obrazovanje", "kvalitetnim"]
                )
            ][:2]
            if general_info:
...
Odgovori kratko i jasno na srpskom jeziku koristeći samo informacije iz konteksta."""
        ...
        response = requests.post(
            f"{ENDPOINT}/chat/completions", headers=headers, json=data, timeout=30
        )
...
@app.route("/")
def home():
    return jsonify({"message": "NLP servis je aktivan!", "endpoints": ["/search", "/status"]})


@app.route("/status")
def status():
    return jsonify({"status": "active", "service": "NLP Chat Service", "version": "1.0"})
    ...
    return jsonify(
        {
            ...
            "knowledge_base_preview": KNOWLEDGE_BASE[:200] + "..."
            if len(KNOWLEDGE_BASE) > 200
            else KNOWLEDGE_BASE,
...
def test_api():
    ...
    try:
        ...
        data = {
            "messages": [{"role": "user", "content": "Reci zdravo na srpskom jeziku."}],
            "model": MODEL_NAME,
            "temperature": 0.3,
            "max_tokens": 50,
        }
...
def search():
    try:
        ...
        return jsonify({"response": ai_response, "context_used": relevant_content, "query": query})
